...
class PatternHackerService {
  ...
  Future<Map<String, dynamic>?> getLatestSessionForChild(int childId) async {
    ...
    print(
      'PATTERN HACKER: latest session response = '
  ...
  Future<List<PatternHackerLevelModel>> getLevelsByActivity(
    int activityId,
  ) async {
    ...
    print('PATTERN HACKER: get levels url = $uri');

    final response = await _apiClient.get(uri);

    print(
      'PATTERN HACKER: get levels response = '
      '${response.statusCode} - ${_shortBody(response.body)}',
    );
    ...
    final levels = PatternHackerLevelModel.listFromPageResponse(data);
