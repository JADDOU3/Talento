...
/// API layer for Cognitive Maze — mirrors BodilyMazeService exactly (same
/// AuthApiClient pattern already used across every other activity service).
class CognitiveMazeService {
  ...
  Future<List<CognitiveMazeLevel>> getLevels(int activityId) async {
    final url =
        '${ApiConstants.levelsByActivity(activityId)}?page=0&size=100&sort=levelNumber,asc';
    debugPrint('COGNITIVE MAZE GET LEVELS: $url');

    final res = await _client.get(Uri.parse(url));
    debugPrint('COGNITIVE MAZE LEVELS: ${res.statusCode} - ${res.body}');
    ...
    if (res.statusCode >= 200 && res.statusCode < 300) {
      ...
      return content
          .whereType<Map>()
          .map((e) =>
          CognitiveMazeLevel.fromJson(Map<String, dynamic>.from(e)))
          .toList();
    }
  ...
  Future<int> createLevelAttempt({
    ...
    debugPrint('COGNITIVE MAZE CREATE ATTEMPT: $body');

    final res = await _client.post(
      Uri.parse(ApiConstants.levelAttempts),
      body: jsonEncode(body),
    );
    debugPrint('COGNITIVE MAZE ATTEMPT RESP: ${res.statusCode} - ${res.body}');
  ...
  Future<void> updateLevelAttempt({
    ...
    debugPrint('COGNITIVE MAZE UPDATE ATTEMPT: $body');
    ...
    debugPrint(
      'COGNITIVE MAZE UPDATE ATTEMPT RESP: ${res.statusCode} - ${res.body}',
    );
  ...
  Future<void> logActivityEvent({
    ...
    debugPrint('COGNITIVE MAZE ACTIVITY EVENT RESP: ${res.statusCode} - ${res.body}');
  ...
  Future<void> logLevelEvent({
    ...
    debugPrint('COGNITIVE MAZE LEVEL EVENT RESP: ${res.statusCode} - ${res.body}');
