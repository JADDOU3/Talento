...
class EmotionChainService {
  ...
  Future<Map<String, dynamic>?> getLatestSessionForChild(int childId) async {
    ...
    print(
      'EMOTION CHAIN: latest session response = '
  ...
  Future<List<EmotionChainLevelModel>> getLevelsByActivity(
    int activityId,
  ) async {
    ...
    print('EMOTION CHAIN: get levels url = $uri');

    final response = await _apiClient.get(uri);

    print(
      'EMOTION CHAIN: get levels response = '
      '${response.statusCode} - ${_shortBody(response.body)}',
    );
    ...
    final levels = EmotionChainLevelModel.listFromPageResponse(data);
