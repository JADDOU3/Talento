...
import 'bodily_maze_intro.dart';

/// Prepares the Bodily Maze session, then shows BodilyMazeIntro.
///
/// Reuses ColorLabContextService.resolveFromKnown — the entry pipeline
/// (selected child → kit → progress → session → activity session) is
/// identical across tilt/colour activities, so there's no reason to
/// duplicate it. Open THIS screen from the roadmap.
class BodilyMazeLauncher extends StatefulWidget {
  ...
  @override
  State<BodilyMazeLauncher> createState() => _BodilyMazeLauncherState();
}

class _BodilyMazeLauncherState extends State<BodilyMazeLauncher> {
